...
const Home = () => {
  ...
  // --- LOGIC LẤY 6 BÁC SĨ RATING CAO NHẤT (Tăng từ 4 lên 6) ---
  useEffect(() => {
    const q = query(
      collection(db, "users"),
      where("role", "==", "doctor"),
  ...
  // Cập nhật lại giao diện sao cho bé hơn để nhét gọn vào thẻ nhỏ
  const renderStars = (rating) => {
    const fullStars = Math.floor(rating);
    return (
      <div style={{ display: "flex", justifyContent: "center", gap: "2px" }}>
        {[...Array(5)].map((_, index) => (
          <span key={index} style={{ color: index < fullStars ? "#ffc107" : "#e4e5e9", fontSize: "14px" }}>★</span>
        ))}
      </div>
    );
  };
  ...
  return (
    <main style={{ minHeight: '100vh', backgroundColor: '#f8f9fa' }}>
      ...
      {/* SECTION 5: ĐỘI NGŨ BÁC SĨ (ĐÃ THU NHỎ CARD) */}
      <section style={{ padding: '70px 20px', background: '#ffffff' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          ...
          {/* Grid hiển thị danh sách bác sĩ ĐÃ THU NHỎ */}
          <div style={{ 
            display: 'grid',
            gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', // <-- Chìa khóa thu nhỏ: minmax 180px
              ...
              doctors.map((doctor) => (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: '16px', // <-- Bo góc gọn hơn
                    textAlign: 'center',
                ...
                >
                  ...
                  <span style={{
                    ...
                    color: 'white',
                    padding: '3px 8px', // <-- Thu nhỏ text badge
                    borderRadius: '12px',
                    fontSize: '10px',
                    fontWeight: 'bold'
                  }}>
                  ...
                  <img
                    ...
                    alt={doctor.name}
                    style={{
                      width: '75px', // <-- Thu nhỏ ảnh
                      height: '75px', // <-- Thu nhỏ ảnh
                      borderRadius: '50%',
                      margin: '0 auto 12px',
                      objectFit: 'cover',
                  ...
                  <h4 style={{ 
                    ...
                    fontWeight: '700', 
                    color: '#2d3436', 
                    fontSize: '1.05rem', // <-- Cỡ chữ bé lại 
                    whiteSpace: 'nowrap', 
                    overflow: 'hidden', 
                  ...
                  <p style={{ 
                    ...
                    fontWeight: '600',
                    marginBottom: '8px',
                    fontSize: '0.85rem', // <-- Cỡ chữ bé lại
                    whiteSpace: 'nowrap', 
                    overflow: 'hidden', 
                    textOverflow: 'ellipsis' 
                  }}>
                  ...
                  <div style={{ marginBottom: '10px' }}>
                    ...
                    <div style={{ fontSize: '0.75rem', color: '#636e72', marginTop: '2px' }}>
                      <strong>{doctor.rating ? doctor.rating.toFixed(1) : "0.0"}</strong> ({doctor.reviewCount || 0})
                    </div>
                  </div>

                  <p style={{ 
                    fontSize: '0.8rem', // <-- Cỡ chữ bé lại
                    color: '#7f8c8d',
                    marginBottom: '15px',
                    height: '35px', // <-- Chiều cao text thấp lại
                    overflow: 'hidden',
                    lineHeight: '1.4'
                  }}>
