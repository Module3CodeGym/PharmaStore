...
const Home = () => {
  ...
  const handleCategoryClick = (categoryName) => {
    ...
    if (categoryFilter === categoryName) {
      newParams.delete('cat'); // Bỏ lọc nếu click lại category đang chọn
    } else {
  ...
  return (
    <main style={{ minHeight: '100vh', backgroundColor: '#f8f9fa' }}>
      ...
      <section style={{ padding: '60px 20px' }} id="categories">
          ...
          {categories.map(cat => (
            <div
              ...
              style={{
                ...
                cursor: 'pointer',
                transition: 'all 0.3s',
                border: categoryFilter === cat.name ? '3px solid #00b894' : '3px solid transparent',
                transform: categoryFilter === cat.name ? 'scale(1.05)' : 'scale(1)',
                boxShadow: categoryFilter === cat.name ? '0 10px 20px rgba(0,0,0,0.1)' : 'none'
              }}
            >
              <div style={{ fontSize: '2.5rem', marginBottom: '10px' }}>{cat.icon}</div>
              <h4 style={{ fontSize: '0.9rem', fontWeight: categoryFilter === cat.name ? 'bold' : 'normal' }}>{cat.name}</h4>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '60px 20px', background: '#f8f9fa' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
            <h2 style={{ borderLeft: '5px solid #007bff', paddingLeft: '15px' }}>
              {searchQuery ? `Kết quả tìm kiếm cho "${searchQuery}"` : (categoryFilter ? `Sản phẩm thư mục "${categoryFilter}"` : "Sản Phẩm Bán Chạy 🔥")}
            </h2>
          ...
          {filteredProducts.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px', background: 'white', borderRadius: '15px' }}>
              ...
              <p style={{ color: '#666' }}>Vui lòng thử từ khóa khác hoặc xóa bộ lọc danh mục.</p>
              <button
                onClick={() => setSearchParams({})}
                style={{ marginTop: '20px', padding: '10px 20px', background: '#00b894', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer' }}
              >
                Xóa tất cả bộ lọc
              </button>
            </div>
